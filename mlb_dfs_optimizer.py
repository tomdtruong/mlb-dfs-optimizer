...
with col_sal1:
    min_salary = st.number_input("Min Salary ($)", value=48800, min_value=40000, max_value=50000, step=100)
with col_sal2:
...
max_roster_own = st.sidebar.slider("Max Cumulative Ownership (%)", min_value=80.0, max_value=250.0, value=160.0, step=5.0)
...
with tab_heatmap:
    ...
    pivot_proj = df[df["Position"] != "P"].pivot_table(
        index="Team", 
    ...
    team_stats = df.groupby(["Team", "Opponent"]).agg(
        ImpliedRuns=("ImpliedRuns", "first"),
        TotalHitterProj=("Projection", lambda x: x[df.loc[x.index, "Position"] != "P"].sum()),
        AvgOwnership=("Ownership", "mean")
    ).reset_index()
...
# -----------------------------------------------------------------------------
def optimize_dk_mlb(df_input, stack_type, min_sal, max_sal, block_p_h, block_p_p, max_own, num_lineups_to_gen, max_overlap_val):
    df_raw = df_input.copy()
    
    # 1. Automatic Pre-filtering to Eliminate Solver Bottlenecks:
    # Filter out inactive or 0-projection players
    df_active = df_raw[df_raw["Projection"] > 0].copy()
    
    # Filter out 400+ inactive relief pitchers: keep top 3 pitchers per team by projection
    pitchers_filtered = df_active[df_active["Position"] == "P"].groupby("Team", as_index=False).apply(
        lambda g: g.nlargest(3, "Projection")
    ).reset_index(drop=True)
    
    hitters_filtered = df_active[df_active["Position"] != "P"].reset_index(drop=True)
    ...
    for l_idx in range(num_lineups_to_gen):
        ...
        # 2. Positional constraints: exactly 1 player per slot
        for slot in slots:
        ...
        # 3. Total Lineup Size: Exactly 10 players
        ...
        # 8. Max Overlap Diversity Constraint
        ...
        # Fast Solver invocation with 10s timeout and 1% optimality gap
...
with tab_opt:
    ...
    with col_info:
        st.write(f"Targeting: **{stack_strategy}** | Salary: **${min_salary:,} - ${max_salary:,}** | Max Overlap: **{max_overlap}**")
